        ...
        for i in  range(20):
            ...
            for y in y_png:
                for pq in pq_png:
                    ...
                    # else:
                    #     threshold_num = 0.7
                    is_pq = self.utils.compare_image_region(template_path=pq, region=(597, y, 150, 150), page_name=pq, threshold=0.7, is_click=False)
                    if is_pq is True:
                        if y == y_png[0]:
                        ...
                        time.sleep(1)
                        is_no_tl = self.utils.compare_image_region(template_path="../page_png/pq/no_tl.png", region=(200, 820, 600, 80), page_name="无派遣体力", threshold=0.7, is_click=False)
                        if is_no_tl is True:
                            print("无体力派遣")
                            return True
                        time.sleep(0.5)
                        is_paichu = self.utils.get_snapshot(file_path="../page_png/paichu.png", compare=True, page_name='手动派遣页面')
            ...
            self.utils.compare_image_region(template_path="../page_png/pq/tishi.png", region=(550, 1130, 400, 130), page_name="稀有度提示弹窗")
            print(f"第{i+1}次循环结束")
