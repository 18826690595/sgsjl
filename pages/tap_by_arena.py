...
class Tap_By_Arena(Utils):
    ...
    def Page_Arena(self):
        ...
        if is_home is True:
            ...
            # 修正错误的坐标值
            self.coordinates(width=0.5, height=0.78)  # 原为 height=78

            for i in range(5):
                time.sleep(1)
                self.coordinates(width=0.73, height=0.53)
                self.coordinates(width=0.73, height=0.56)
                time.sleep(1.5)
